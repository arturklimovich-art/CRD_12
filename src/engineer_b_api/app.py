# -*- coding: utf-8 -*-
from fastapi import FastAPI, Request`nfrom fastapi.responses import ORJSONResponse
...
print("[DEBUG] Starting Truth System import...")
try:
    from routes.truth import router as truth_router
    TRUTH_AVAILABLE = True
    print("[DEBUG] Truth System import SUCCESS, TRUTH_AVAILABLE=True")
except Exception as e_truth:
    print(f"[DEBUG] Truth System import failed: {e_truth}")
...
app = FastAPI(`n    title="Engineer B API",`n    version="4.1 - Self-Healing + Roadmap",`n    default_response_class=ORJSONResponse`n)
...
# Register Truth System router
if TRUTH_AVAILABLE:
    app.include_router(truth_router)
    print("[INFO] Truth System routes registered")
...
# === TRUTH MATRIX HTML UI ===
if TRUTH_AVAILABLE:
    @app.get('/truth/matrix', response_class=HTMLResponse)
    async def truth_matrix_page(request: Request):
        """Truth Matrix Dashboard - Web UI"""
        return templates.TemplateResponse(
            "truth_matrix.html",
            {"request": request, "title": "Truth Matrix - CRD12"}
        )
    print("[INFO] Truth Matrix HTML UI registered")
...
try:
    from fastapi import FastAPI, Request`nfrom fastapi.responses import ORJSONResponse as _FastAPI
