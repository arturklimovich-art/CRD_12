# ИСПРАВЛЕННАЯ ВЕРСИЯ ATTACH-CANDIDATEPATCH С ИНТЕГРАЦИЕЙ KURATOR
# Исправлены синтаксические ошибки интерполяции строк
...
function Attach-CandidatePatch {
    ...
    if (-not $validationResult.IsSafe) {
        ...
        Write-Host "📝 [EVENT] CANDIDATE_VALIDATION_FAILED: JobId=$JobId" -ForegroundColor Red
        Write-Host "   Security Level: $($validationResult.SecurityLevel)" -ForegroundColor Gray
        Write-Host "   Issues: $($validationResult.Issues.Count) detected" -ForegroundColor Gray
    ...
    Write-Host "📝 [EVENT] CANDIDATE_PATCH_ATTACHED: JobId=$JobId" -ForegroundColor Gray
    ...
    try {
        ...
        Write-Host "✅ Candidate patch attached to new task: $($patchedTask.task_id)" -ForegroundColor Green
        ...
        Write-Host "❌ Failed to create task for candidate patch: $($_.Exception.Message)" -ForegroundColor Red
