        ...
        if smoke_test_ok and runtime_smoke_ok and generated_code:
            if target_file:
                ...
                if applied_ok:
                    ...
                    # Подготовим ожидание пост-рестарта и проверку (best-effort)
                    try:
                        expected_sha = sha256_str(generated_code)
                        wait_timeout = int(os.getenv("POST_RESTART_TIMEOUT", "120"))
                        logger.info("Waiting up to %s seconds for post-restart validation (expected sha=%s)...", wait_timeout, expected_sha[:16])
                        # Вызов wait реализован асинхронно
                        post_ok, post_msg = await _wait_for_post_restart(target_file, expected_sha, timeout_seconds=wait_timeout)
                        if post_ok:
                            logger.info("Post-restart validation succeeded: %s", post_msg)
                            try:
                                put_event(stage="deploy", event_type="post_deploy_validated", context={"status": "ok", "validated_at": datetime.now().isoformat(), "target_file": target_file})
                            except Exception:
                                pass
                            report_data["description"] = f"Patch successfully applied to {target_file}. {apply_msg}. Restart validated."
                            report_data["tests_status"] = "passed"
                            report_data["deployment_ready"] = True
                            report_data["smoke_test_result"] = runtime_smoke_msg
                        else:
                            logger.warning("Post-restart validation FAILED: %s", post_msg)
                            # Откат
                            if backup_path and os.path.exists(backup_path):
                                try:
                                    ...
                                    logger.warning("Rollback: restored %s from %s", target_file, backup_path)
                                    try:
                                        put_event(stage="rollback", event_type="rolled_back", context={"restored_from": backup_path, "target": target_file})
                                    except Exception:
                                        pass
                                except Exception as rb_err:
                                    logger.error("Rollback failed: %s", rb_err)
                            try:
                                put_event(stage="deploy", event_type="post_deploy_failed", context={"reason": post_msg, "target_file": target_file})
                            except Exception:
                                pass
                            final_status_for_orchestrator = "failed"
                            report_data["description"] = f"Patch applied but post-restart validation failed: {post_msg}. Rolled back if possible."
                            report_data["tests_status"] = "error"
                            report_data["deployment_ready"] = False
                            report_data["smoke_test_result"] = runtime_smoke_msg
                    except Exception as e_post:
                        logger.exception("Error during post-restart validation: %s", e_post)
                        try:
                            put_event(stage="deploy", event_type="post_deploy_failed", context={"reason": str(e_post), "target_file": target_file})
                        except Exception:
                            pass
                        final_status_for_orchestrator = "failed"
                        report_data["description"] = f"Patch applied but post-restart validation errored: {e_post}"
                        report_data["tests_status"] = "error"
                        report_data["deployment_ready"] = False
                else:
                    final_status_for_orchestrator = "failed"
...
async def apply_manual_patch_inline(patch_id: str, approve_token: str = Body(None)):
    # временная заглушка для E1-PATCH-MANUAL
    return {
        "patch_id": patch_id,
...
@app.post("/api/patches/{patch_id}/apply")
async def apply_manual_patch_inline(patch_id: str, approve_token: str = Body(None)):
    # читаем патч из БД через psql CLI (простая версия)
    # NOTE: в проде это лучше вынести в отдельный модуль и использовать пул подключений
    ...
    # ожидаем формат: id|approve_token|status
    parts = line.split("|")
    db_id = parts[0]
    db_token = parts[1] if len(parts) > 1 else ""
    db_status = parts[2] if len(parts) > 2 else ""

    # сверяем токены и статус
    if db_status != "approved":
        return {
            "patch_id": patch_id,
            "status": "rejected",
            "reason": f"status={db_status}",
            "source": "app.py"
        }
    ...
    if db_token and approve_token != db_token:
        return {
            "patch_id": patch_id,
            "status": "rejected",
            "reason": "approve_token mismatch",
            "source": "app.py"
        }

    # если всё ок — пишем событие started
    insert_evt_sql = f"INSERT INTO eng_it.patch_events (patch_id, event_type, payload) VALUES (''{patch_id}'',''eng.apply_patch.started'',''{{\"by\":\"engineer_b_api\"}}''::jsonb);"
    Popen([
        "psql",
        "-U", "crd_user",
        "-d", "crd12",
    ...
    # тут будет место, где реально применяем патч (копирование файлов) — пока заглушка
    # финальное событие
    insert_done_sql = f"INSERT INTO eng_it.patch_events (patch_id, event_type, payload) VALUES (''{patch_id}'',''eng.apply_patch.finished'',''{{\"result\":\"simulated\"}}''::jsonb);"
    Popen([
        "psql",
        "-U", "crd_user",
        "-d", "crd12",
