-- Миграция: создание системы Kurator для проверки политик и допусков

-- Таблица политик
CREATE TABLE IF NOT EXISTS nav.kurator_policies (
    policy_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    policy_name TEXT NOT NULL UNIQUE,
    policy_type TEXT NOT NULL CHECK (policy_type IN ('roadmap_revision', 'technical_task', 'security', 'path_validation')),
    description TEXT,
...
-- Индексы для политик
CREATE INDEX idx_kurator_policies_type ON nav.kurator_policies (policy_type, active);
CREATE INDEX idx_kurator_policies_severity ON nav.kurator_policies (severity, active);

-- Таблица результатов проверок
CREATE TABLE IF NOT EXISTS nav.kurator_checks (
    check_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    policy_id UUID NOT NULL REFERENCES nav.kurator_policies(policy_id),
    target_type TEXT NOT NULL CHECK (target_type IN ('roadmap_revision', 'technical_task')),
    target_id UUID NOT NULL, -- revision_id или tz_id
...
CREATE INDEX idx_kurator_checks_target ON nav.kurator_checks (target_type, target_id);
CREATE INDEX idx_kurator_checks_time ON nav.kurator_checks (checked_at);
...
-- Добавляем начальные политики
INSERT INTO nav.kurator_policies (policy_name, policy_type, description, rule_condition, severity) VALUES
('no_deletions', 'roadmap_revision', 'Запрет операций удаления в Roadmap', '{"operation": "check_deletion"}'::JSONB, 'blocking'),
('valid_paths', 'security', 'Проверка корректности путей к файлам', '{"operation": "check_paths"}'::JSONB, 'blocking'),
('item_exists', 'roadmap_revision', 'Проверка существования элемента Roadmap', '{"operation": "check_item_existence"}'::JSONB, 'blocking'),
('has_acceptance_criteria', 'technical_task', 'Проверка наличия критериев приемки', '{"operation": "check_acceptance"}'::JSONB, 'blocking'),
('valid_artifacts', 'technical_task', 'Проверка корректности артефактов', '{"operation": "check_artifacts"}'::JSONB, 'warning');

-- Комментарии
COMMENT ON TABLE nav.kurator_policies IS 'Политики и правила проверки Kurator';
COMMENT ON TABLE nav.kurator_checks IS 'Результаты проверок Kurator';
COMMENT ON FUNCTION nav.kurator_check_roadmap_revision IS 'Проверка ревизии Roadmap на соответствие политикам';
COMMENT ON FUNCTION nav.kurator_check_technical_task IS 'Проверка технического задания на соответствие политикам';
COMMENT ON FUNCTION nav.kurator_validate_roadmap_revision IS 'Комплексная проверка ревизии Roadmap с сохранением результатов';
